...
public class StructureParticleBox implements StructureComponent, StoredStructureComponent {
    @Override
    public void onActiveCreated(@NotNull ActiveStructure structure) {
        structure.set(CruxCoreComponents.ACTIVE_STRUCTURE_PARTICLE_BOX, new ActiveStructureParticleBox(structure));
    }
    ...
    public void onCreated(@NotNull StoredChunk chunk, @NotNull CruxPosition center, double rotation, @NotNull StoredStructure stored) {
        stored.set(CruxCoreComponents.STRUCTURE_PARTICLE_BOX, this);
    }
}
